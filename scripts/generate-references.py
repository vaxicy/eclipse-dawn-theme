"""Compose every Eclipse Dawn store asset from one HTML/CSS source.
...
.bookmarks .sep{width:1px;height:14px;background:rgba(236,236,238,.14)}
...
.tile{width:440px;height:280px;background:var(--frame);position:relative;overflow:hidden;text-align:center;color:var(--ntptext)}
.tile img{width:78px;height:78px;display:block;margin:26px auto 0}
.tile h1{font-family:Georgia,serif;font-weight:normal;font-size:37px;margin:14px 0 0;color:var(--ntptext)}
.tile .kicker{font-size:13px;letter-spacing:5px;margin-top:9px;color:var(--link)}
.tile p{font-size:14px;margin:19px 0 0;color:#C9C9D6}
...
tile = ('<div class="tile">' + f'<img src="{LOGO_URI}" alt="Eclipse Dawn logo">'
        ...
        + '<p>A dark, serene space for focused work.</p></div>')
marquee = ('<div class="marquee"><h1>Eclipse Dawn Theme</h1>'
           '<p>Deep indigo, eclipse navy and a calm periwinkle accent.</p>'
           + '<div class="frame">' + window(height=500) + '</div></div>')

PALETTE = [
    ('Deep Indigo', 'frame', 'Window frame & tab strip', 'ntp_text'),
    ('Charcoal Blue', 'toolbar', 'Toolbar, bookmarks & active tab', 'ntp_text'),
...
intro = ('<div class="intro"><div class="kicker">A QUIET, DARK PALETTE</div><h1>Eclipse Dawn Theme</h1>'
         ...
         '<p class="chips">Solid colors · Minimal design · Dark interface · Tuned for contrast</p></div>')
